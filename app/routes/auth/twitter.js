...
function twitterStrategy() {
  function buildTwitterProfile(accessToken, refreshToken, profile, done) {
    var email = profile.username + '@user.twitter.com';
    ...
    function associateEmail(err, user) {
      ...
      findVerifier('twitter', function (err, verifier) {
        if (err) {
          return done(err);
        }
